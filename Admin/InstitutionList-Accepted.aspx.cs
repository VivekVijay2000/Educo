...
public partial class Admin_Default : System.Web.UI.Page
{
   ...
   // SqlConnection con = new SqlConnection("Data Source=DREAM;Initial Catalog=db_student;Integrated Security=True");
    ...
    {
        string sel = "select * from tbl_institution i inner join tbl_place p on p.place_id=i.place_id inner join tbl_district d on d.district_id=p.district_id inner join tbl_institutiontype t on t.institutiontype_id=i.institutiontype_id where i.institution_status=1";
    ...
    protected void Grd_institutionlist_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "re")
        {
            string up2 = "update tbl_institution set institution_status='2' where institution_id='" + id + "'";
            SqlCommand cmd = new SqlCommand(up2, con);
            cmd.ExecuteNonQuery();
            fillgrid();
        }
